...
export const VideoControls: React.FC<VideoControlsProps> = ({
  ...
  return (
    <div className="flex-shrink-0 bg-[#0a0a12] border-t border-[#1e1e2e] px-5 py-2 select-none">
      ...
      >
        ...
        {!summaryLoading && normalizedBuckets.length > 0 && totalTime > 0 && (
          ...
          >
            {normalizedBuckets.map((b, i) => {
              ...
              return (
                <g key={i}>
                  {/* Home bar — upward from midline */}
                  {homeH > 0 && (
                    <rect
                      x={i} y={mid - homeH} width={0.8} height={homeH}
                      fill={homeColor} opacity={0.6}
                    />
                  )}
                  {/* Away bar — downward from midline */}
                  {awayH > 0 && (
                    <rect
                      x={i} y={mid} width={0.8} height={awayH}
        ...
        {/* Centre line */}
        <div
          className="absolute left-0 right-0 bg-[#252535]"
          style={{ top: waveformHeight / 2, height: 1 }}
        />

        {/* Progress fill overlay (amber tint over played portion) */}
        <div
          className="absolute top-0 bottom-0 left-0 bg-amber-500/10 pointer-events-none transition-none"
          style={{ width: `${progress * 100}%` }}
        />

        {/* Progress line */}
        <div
          className="absolute top-0 bottom-0 w-0.5 bg-amber-400 pointer-events-none shadow-[0_0_6px_rgba(245,158,11,0.8)]"
        ...
        {goalMarkers.map((g, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Goal icon at top */}
              <div
                className="absolute text-[8px] leading-none -translate-x-1/2 font-bold"
                style={{ top: isHome ? 2 : undefined, bottom: isHome ? undefined : 2, color }}
              >
                ⚽
              </div>
            </div>
        ...
        {/* Half-time line (at 45 min / 2700s) */}
        {totalTime > 2700 && (
          <div
            className="absolute top-0 bottom-0 w-px bg-[#444460]/60 pointer-events-none"
            style={{ left: `${(2700 / totalTime) * 100}%` }}
          />
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          <SkipBtn label="−30s" disabled={!canControl}
            onClick={() => onSeek(Math.max(0, matchTime - 30))} title="Rewind 30 seconds" />
          <SkipBtn label="−10s" disabled={!canControl}
            onClick={() => onSeek(Math.max(0, matchTime - 10))} title="Rewind 10 seconds" />
        </div>

        {/* Play / Pause */}
        <button
          onClick={running ? onPause : onPlay}
          disabled={!connected || matchEnded}
        ...
        <div className="flex items-center gap-1">
          <SkipBtn label="+10s" disabled={!canControl}
            onClick={() => onSeek(matchTime + 10)} title="Skip 10 seconds" />
          <SkipBtn label="+30s" disabled={!canControl}
            onClick={() => onSeek(matchTime + 30)} title="Skip 30 seconds" />
        </div>

        {/* Time display — match clock + wall-clock */}
        <div className="font-mono text-xs text-gray-400 tabular-nums ml-1 flex items-baseline gap-1.5 min-w-[110px]">
          <span className="text-gray-200 text-sm font-semibold">
            {fmtMatchClock(matchTime, currentPeriod)}
          </span>
        ...
        <button
          ...
          className={`w-8 h-8 flex items-center justify-center rounded-lg border transition-all
            ${muted
              ? 'border-red-500/40 text-red-400 bg-red-900/15'
              : 'border-[#1e1e2e] text-gray-500 hover:text-gray-200 hover:border-[#2e2e45]'
            }`}
        >
          {muted ? (
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg border border-[#1e1e2e] font-mono text-[11px] text-gray-500
