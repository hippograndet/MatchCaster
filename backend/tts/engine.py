...
from tts.voices import get_voice_for_agent, PIPER_VOICES_DIR, FALLBACK_VOICE, MACOS_SAY_VOICES
...
class PiperTTSEngine:
    ...
    def synthesize_sync(self, text: str, agent_name: str) -> Optional[bytes]:
        """
        ...
        if self._piper_bin:
            ...
            wav = self._piper_synthesize(text, voice)
            if wav:
                return wav
        ...
        if self._say_ok:
            ...
            return _synthesize_say(text, say_voice)
    ...
    async def synthesize(self, text: str, agent_name: str) -> Optional[bytes]:
        """Async wrapper: runs synthesis in thread pool executor."""
        loop = asyncio.get_event_loop()
        try:
            wav_bytes = await loop.run_in_executor(
                None, self.synthesize_sync, text, agent_name
            )
            return wav_bytes
        except Exception as exc:
